    ...
    this.validator.validate('project', project, function(errors) {
      ...
      if (!errors) {
        if (that.helpers.isNew(project)) {
          ...
          that.update({ _id: that.ObjectId(project._id) }, {
            $set: { code: project_code, name: project.name }
          });
          //fetch the created project
          that.findOne({ _id: that.ObjectId(project._id) }, function(err, updated_project) {
        ...
        if (that.helpers.isNew(project)) {
          ...
          that.findOne({ _id: that.ObjectId(project._id) }, function(err, fetched_project) {
            if (fetched_project) {
